...
const CommentSearch: React.FC<CommentSearchProps> = React.memo(({ comments }) => {
  ...
  return (
    <Box sx={{marginTop: 2 }}>
      ...
      <TableContainer sx={{ maxWidth: '400px', maxHeight: '250px', overflowY: 'auto' }}>
        <Table>
          <TableHead>
            <TableRow>
              <TableCell  sx={{ padding: '8px', fontSize: '12px' }}>時間</TableCell>
              <TableCell  sx={{ padding: '8px', fontSize: '12px' }}>コメント</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {filteredComments.length > 0 ? (
              filteredComments.map((comment) => (
                <TableRow key={comment.no}>
                  <TableCell  sx={{ padding: '8px', fontSize: '12px' }}>{formatVposToTime(comment.vpos)}</TableCell>
                  <TableCell  sx={{ padding: '8px', fontSize: '12px' }}>{comment.message}</TableCell>
                </TableRow>
              ))
            ) : (
              <TableRow>
                <TableCell colSpan={2} align="center"  sx={{ padding: '8px', fontSize: '12px' }}>
                  検索結果がありません
                </TableCell>
              </TableRow>
            )}
